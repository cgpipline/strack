...
class RelationModel extends Model
{
    ...
    protected function formatFieldData($formatParam, $value, $formatMode = 'grid')
    {
        if ($formatParam["type"] === "show_from") {
            ...
            switch ($formatParam["format"]) {
                ...
                case "combobox":
                    // 自定义字段特有
                    return !empty($value) && array_key_exists($value, $formatParam["data"]) ? $formatParam["data"][$value] : '';
                case "tag_type":
                    // tag 类型语言包
    ...
    private function getHasManyRelationQueryData($hasManyRelationConfig, $primaryKey, $primaryIdsString, $primaryMapData, $formatMode = "grid")
    {
        ...
        foreach ($hasManyRelationConfig['has_many_relation'] as $key => $relationItem) {
            ...
            if (is_array($relationItem["fields"]) && $hasManyRelationConfig["master_alias_name"] === "entity" && $relationItem["module_code"] === "base") {

                // 实体下面工序任务数据单独处理
                if($formatMode === 'grid'){
                    $primaryKey = "{$hasManyRelationConfig["master_alias_code"]}_id";
                }else{
                    $primaryKey = "base_entity_id";
                }
    ...
    public function buildFilterData($currentModule, $relationStructure, $filter)
    {
        ...
        $filterRequest = !empty($filter["request"]) ? $this->buildItemFilter($currentModule, $relationStructure, $filter["request"], "AND", 'request') : [];
    ...
    private function buildSortRule($sortRule, $groupRule = [])
    {
        if (!empty($sortRule) || !empty($groupRule)) {
            $sort = [];
            $sortRules = array_merge($groupRule, $sortRule);
            ...
            foreach ($sortRules as $ruleItem) {
                if (array_key_exists("module_code", $ruleItem) && !empty($ruleItem["module_code"])) {
                    if ($ruleItem["field_type"] === "built_in") {
                        ...
                        $type = $variableModel->where([
                            'module_id' => $moduleIds[$ruleItem['module_code']],
                            'code' => $ruleItem['field']
                        ])->getField('type');
                        if (!in_array($type, ['horizontal_relationship', 'belong_to'])) {
    ...
    private function buildWidgetFilter($item)
    {
        ...
        switch ($editor) {
            ...
            case "timespinner":
                ...
                $this->isNullOrEmptyFilter = false;
                break;
            case "combobox":
            case "tagbox":
    ...
    private function buildDirectWidgetFilter(&$filterData, $item, $queryKey, $primaryModuleKey, $currentModule, $logic)
    {
        if ($item["field_type"] == "custom") {
            ...
            if (in_array($item['condition'], ['LIKE'])) {
                $queryFieldKey = $item["field"];
            } else {
                $queryFieldKey = $queryKey;
            }

            $queryMap = [$queryFieldKey => $this->buildWidgetFilter($item)];
        }
    ...
    private function buildFilterInData($filterData, $field, $condition)
    {
        if (!empty($filterData)) {
            ...
            return [$this->getFilterCondition("IN"), join(",", $filterData)];
        } else {
                    ...
                    if (array_key_exists($item["module_code"], $relationStructureFilterMap)) {
                        switch ($relationStructureFilterMap[$item["module_code"]]["mapping_type"]) {
                            case "master":
                                ...
                                $buildFilterData = $this->buildWidgetFilter($item);

                                if ($this->isNullOrEmptyFilter) {
                                    $filterData[$filterKey] = ['IN', join(',', ['', 0])];
                                } else {
                                    $filterData[$filterKey] = $buildFilterData;
                                }
                                break;
                            case "belong_to":
                                // 从属于数据关联过滤条件
     ...
     * @return string
     */
    private function getFilterCondition($condition)
    {
        return $this->isNullOrEmptyFilter ? 'NOT IN' : $condition;
    }
    ...
    private function mergeRemoteCustomFilteringConditions($horizontalFilterKey, $filterData, $newValue)
    {
        if (array_key_exists($horizontalFilterKey, $filterData)) {
            if ($filterData[$horizontalFilterKey] == 0 || $newValue == 0) {
                ...
                return [$this->getFilterCondition($condition), join(',', $resultList)];
            }
        } else {
            ...
            return [$this->getFilterCondition($condition), join(',', $newValue)];
        }
    }
        ...
        if (!empty($filter)) {
            if ($filter["number"] > 1) {
                ...
                if (array_key_exists('logic', $filter)) {
                    ...
                    $filterData["_logic"] = $filter["_logic"];
                }
                return $filterData;
            } else {
                // 一组过滤条件
    ...
    {
        if (array_key_exists('logic', $filterItem)) {
            $logic = $filterItem["logic"];
        } else {
            $logic = $filterItem["_logic"];
        }
        return $this->buildItemFilter($currentModule, $relationStructure, $filterItem, $logic, 'advance');
    }
}
