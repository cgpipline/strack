...
class Cache
{
    ...
    public static function connect(array $options = [], $name = false)
    {
        ...
        self::$driverType = strtolower($type);
        if (false === $name) {
            $name = md5(serialize($options));
        }

        if (true === $name || !isset(self::$instance[$name])) {
            $class = false !== strpos($type, '\\') ? $type : '\\Think\\Cache\\Driver\\' . ucwords($type);
     ...
     * @return false|int
     */
    public static function lPush($key, $value, $cb = null)
    {
        if (self::$driverType !== 'redis') {
            return false;
        }
        return self::$handler->lPush($key, $value, $cb);
    }
     ...
     * @return int
     */
    public static function zAdd($key, $score, $value, $cb = null)
    {
        if (self::$driverType !== 'redis') {
            return false;
        }
        return self::$handler->zAdd($key, $score, $value, $cb);
    }
}
