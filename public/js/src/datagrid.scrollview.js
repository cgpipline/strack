var scrollview = $.extend({}, $.fn.datagrid.defaults.view, {
        ...
        renderGroup: function (target, groupIndex, group, frozen) {
            ...
            if(opts.groupActive){
                ...
                var c_index = opts.columns.length - 1;

                var group_name = '';
                if(typeof(group.value)=='string'){
                    group_name = group.value;
                }else {
                    if(group.value && group.value.hasOwnProperty('rows')){
                        group.value.rows.forEach(function (val) {
                            group_name += val['name'];
                        });
                    }
                }
                ...
                if (!frozen) {
                    ...
                    if (opts.frozenColumns[c_index].length < 2) {
                        table.push(opts.groupFormatter.call(target, group_name, group.rows));
                    }
                    table.push('</span>');
                } else {
                    //冻结
        ...
        onBeforeRender: function (target, rows) {
            ...
            function getGroup(value) {
                for (var i = 0; i < groups.length; i++) {
                    ...
                    if (getValue(group.value) == getValue(value)) {
                        return group;
                    }
                }
                return null;
            ...
            function getValue(val) {
                if(val && val.hasOwnProperty('rows')){
                    // 多维数据分组
                    return val['rows'][0]['name'];
                }
            ...
            function initCss() {
                if (!$('#datagrid-group-style').length) {
                    $('head').append(
                        ...
                        '.datagrid-group{height:25px;overflow:hidden;font-weight:bold;border-bottom:1px solid #ccc;background-color: #E6E6E6}' +
                        '</style>'
                    );
                }
            }
