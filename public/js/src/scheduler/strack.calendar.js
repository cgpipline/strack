...
!function(a, b) {
    ...
    var qe = "周日_周一_周二_周三_周四_周五_周六".split("_"), re = "周日_周一_周二_周三_周四_周五_周六".split("_"), se = "周日_周一_周二_周三_周四_周五_周六".split("_"), te = Jd, ue = Jd, ve = Jd;
...
!function(t) {
    ...
    function Ot(n, i) {
        ...
        // 加载过滤按钮
        function load_filter_bnt()
        {
            jQuery.ajax({
                type: 'POST',
                url: StrackPHP['getCalendarFilterConfig'],
                dataType: "json",
                beforeSend: function () {
                    jQuery('#fc_filter_wrap').append(Strack.loading_dom('white','','c_filter'));
                },
                success: function (data) {
                    jQuery('#st-load_c_filter').remove();

                    var dom = '';

                    // 团队成员
                    var department_members_dom = '';
                    data['department_members'].forEach(function (val) {
                        department_members_dom += filter_item_base_dom('department_members', val);
                    });

                    var department_members_hide = false;
                    if(data['department_members'].length === 0){
                        department_members_hide = true;
                    }

                    dom += filter_with_search('department_members', '团队成员', department_members_dom, department_members_hide);

                    // 填充项目过滤项
                    var p_list_dom = '';
                    data['project_list'].forEach(function (val) {
                        p_list_dom += filter_item_base_dom('project', val);
                    });

                    dom += filter_with_search('project', StrackLang['Project_Name'], p_list_dom);

                    // 填充状态过滤项
                    var sta_list_dom = '';
                    data['status_list'].forEach(function (val) {
                        sta_list_dom += filter_item_base_dom('status', val);
                    });

                    dom += filter_with_search('status', StrackLang['Status'], sta_list_dom);
                    ...
                    // 填充分派者和执行者选人
                    var receive_list_dom = '',
                        sent_list_dom = '';

                    data['user_list'].forEach(function (val) {
                        receive_list_dom += filter_item_base_dom('user_receive', val);
                        sent_list_dom += filter_item_base_dom('user_sent', val);
                    });

                    dom += filter_with_search('user_receive', StrackLang['Executor'], receive_list_dom, true);
                    dom += filter_with_search('user_sent', StrackLang['Distributive_Person'], sent_list_dom, true);

                    jQuery('#fc_filter_wrap').append(dom);
                    ...
                    // 初始化截止时间日期选择框
                    // 初始化开始时间
                    jQuery('#fc_endtime_from').datebox({
                        width: 100,
                        height: 25,
                        ...
                        onSelect: function (date) {
                            ...
                            // 判断是否选择了结束时间
                            var  end_time = jQuery('#fc_endtime_to').datebox("getValue");
                            if(end_time){
                                Strack.handle_calendar_filter(this,'end_time', {
                                    type : 'between',
                                    value: {
                                        start : Strack.date_format(date, "yyyy-MM-dd"),
                                        end : end_time
                                    }
                                }, true);
                            }
                    ...
                    jQuery('#fc_endtime_to').datebox({
                        ...
                        onSelect: function (date) {
                            ...
                            // 判断是否选择了开始时间
                            var  start_time = jQuery('#fc_endtime_from').datebox("getValue");
                            if(start_time){
                                Strack.handle_calendar_filter(this,'end_time', {
                                    type : 'between',
                                    value: {
                                        start : start_time,
                                        end : Strack.date_format(date, "yyyy-MM-dd")
                                    }
                                }, true);
                            }
                        }
        ...
        function filter_type() {
            ...
            var dom = '';
            var type_list = [
                {id: 'my_receive' , name: StrackLang['Tasks_I_Accept']},
                {id: 'my_sent' , name: StrackLang['Tasks_I_Assignment']},
                {id: 'my_create' , name: StrackLang['Tasks_I_Create']}
            ];

            var type_dom = filter_item_full_choice('task_type');
            type_list.forEach(function (val) {
                type_dom += filter_item_base_dom('task_type', val);
            });

            dom += '<div class="ui dropdown fc-filter-d">' +
                StrackLang['Type']+' <i class="dropdown icon"></i>' +
                '<div class="menu">' +
                type_dom +
                '</div>' +
                '</div>';
        ...
        function filter_end_time() {
            ...
            var time_dom = filter_item_full_choice('end_time');;
            time_list.forEach(function (val) {
                time_dom += filter_item_base_dom('end_time', val);
            });

            dom += '<div class="ui dropdown fc-filter-d">' +
                StrackLang['Cutoff_Time']+' <i class="dropdown icon"></i>' +
                '<div class="menu">' +
                time_dom +
                '<div class="fc-endtime-bw">' +
                '<div class="from aign-left"><input id="fc_endtime_from" /></div>'+
                '<div class="symbol aign-left"> - </div>' +
                '<div class="to aign-left"><input id="fc_endtime_to" /></div>' +
            '</div>' +
            '</div>' +
            '</div>';
        ...
        function p() {
            var //t = j.options.contentHeight,
                t = Strack.panel_height('#'+j.options.domParam.dom, j.options.domParam.min_h),
                e = j.options.height;
        ...
        j.bindOptions([ "locale", "monthNames", "monthNamesShort", "dayNames", "dayNamesShort", "firstDay", "weekNumberCalculation" ], function(t, e, n, i, s, o, l) {
            ...
            var n = e.clone();
            t ? n.stripTime().add(j.defaultAllDayEventDuration) :n.add(j.defaultTimedEventDuration);
            if(j.getIsAmbigTimezone()){
                n.stripZone();
            }
    ...
    function _t(e) {
        function n() {
            ...
            if(s){
                if(h){
                    ...
                    h = this.el = t("<div class='fc-toolbar'/>");

                    h.append(r("left"))
                        .append(r("right"))
                        .append(filter_bnt());

                    if(n.param.rule_create === 'yes') {
                        h.append(add_task());
                    }

                    if(n.param.rule_lock_task_plan === 'yes') {
                        h.append(lock_plan_bnt());
                    }
            ...
            //         h.append(r("left")).append(r("right")).append(filter_bnt())
            //             .append(lock_plan_bnt()).append(add_task()).append(r("center")).append('<div class="fc-clear"/>')) :i();
            // }else {
            //     f = n.theme ? "ui" :"fc", s ? (h ? h.empty() :h = this.el = t("<div class='fc-toolbar'/>"),
            //         h.append(r("left")).append(r("right")).append(filter_bnt())
            //             .append(r("center")).append('<div class="fc-clear"/>')) :i();
            // }
        }
        function i() {
        ...
        function add_task() {
            var dom = '';
            dom += '<div class="fc-add-task">' +
                    '<a href="javascript:;" class="ui button blue" onclick="Strack.common_add_task(this)" data-page="my_scheduler"><i class="icon-uniE672 icon-left"></i>'+StrackLang['Add_Task']+'</a>' +
                '</div>';
            return dom;
        }
        // 锁定计划项
        function lock_plan_bnt() {
            var dom = '';
            dom += '<div class="fc-lock-plan">' +
                '<a href="javascript:;" class="ui button blue" onclick="Strack.calendar_lock_plan(this)" data-mode="" data-view="agendaDay" style="display: none"><i class="icon-uniE9B8 icon-left"></i>'+StrackLang['Lock_Task_Plan']+'</a>' +
                '</div>';
            return dom;
        }
        // 过滤按钮
        function filter_bnt() {
            ...
            // dom += '<div class="fc-filter">' +
            //     '<a href="javascript:;" class="fc-filter-bnt fc-filter-bnt-ac" onclick="Strack.filter_fullCalendar_data(this)" data-filter="my_create">我创建的任务</a>'+
            //     '<a href="javascript:;" class="fc-filter-bnt fc-filter-bnt-ac" onclick="Strack.filter_fullCalendar_data(this)" data-filter="my_receive">我接的任务</a>'+
            //     '<a href="javascript:;" class="fc-filter-bnt fc-filter-bnt-ac" onclick="Strack.filter_fullCalendar_data(this)" data-filter="my_sent">我派的任务</a>'+
            //     '<a href="javascript:;" class="fc-filter-bnt" onclick="Strack.filter_fullCalendar_data(this)" data-filter="">清除过滤</a>'+
            //     '</div>';

            dom += '<div id="fc_filter_wrap" class="fc-filter">' +
                '</div>';
            ...
            return s && t.each(s.split(" "), function(n) {
                ...
                t.each(this.split(","), function(n, i) {
                    ...
                    "title" == i ? (o = o.add(t("")), l = !1) :((s = (r.customButtons || {})[i]) ? (u = function(t) {
                        ...
                        var $plan_lock_bnt = jQuery('.fc-lock-plan');
                        switch (i) {
                            case 'agendaDay':
                                $plan_lock_bnt.show();
                                $plan_lock_bnt.attr('data-view', i);
                                break;
                            case 'agendaWeek':
                            case 'agendaMonth':
                            case 'month':
    ...
    var le = jt.EmitterMixin = {
        ...
        render:function() {
            ...
            if(this.options.parentEl.hasClass('fc-agendaDay-view')){
        ...
        position:function() {
            ...
            e += d.scrollTop(), n += d.scrollLeft(), o.viewportConstrain !== !1 && (r = Math.min(r, e + h.outerHeight() - u - this.margin), r = Math.max(r, e + this.margin), s = Math.min(s, n + h.outerWidth() - a - this.margin), s = Math.max(s, n + this.margin));
    ...
    pe.mixin({
            ...
            if (e.length) {
                ...
                t(s).each(function(n, i) {
                    ...
                    if(l){
                        l.data("fc-seg", s);
                        s.el = l;
                        if(r.name === 'agendaDay' || s.event.is_repeated_settlement === 'no'){
                            o.push(s);
                        }
                    }
                });
        ...
        bindSegHandlersToEl:function(t) {
            this.bindSegHandlerToEl(t, "touchstart", this.handleSegTouchStart), this.bindSegHandlerToEl(t, "touchend", this.handleSegTouchEnd),
                ...
                this.bindSegHandlerToEl(t, "dblclick", this.handleSegDBClick), this.bindSegHandlerToEl(t, "contextmenu", this.handleSegContextmenu);
        },
        ...
        handleSegContextmenu: function(t, e){
            var n = this.view.trigger("contextmenu", t.el[0], t.event, e);
            e.preventDefault();
        },
        handleSegDBClick:function(t, e) {
            var n = this.view.trigger("eventDBClick", t.el[0], t.event, e);
            n === !1 && e.preventDefault();
        },
        ...
        handleSegMousedown:function(t, e) {
            if(jQuery(e.target).closest('.widget_input').length === 0){
        ...
        computeEventDrop:function(t, e, n) {
            ...
            //判断当任务是否还有可使用剩余时间
            var l_id = 0;
            if(n.event_type === 'base'){
                l_id = n.item_id;
            }else {
                l_id = n.link_id;
            }

            var c_time = Strack.current_time();


            if(Strack.G.calendarTaskRemainderLime['task_remainder_end_time'][l_id] < c_time){
                layer.msg('当前任务已经超期，请联系负责人!', {icon: 2, time: 1200, anim: 6});
                return false;
            }

            // if(n.allDay && Strack.G.calendarTaskRemainderLime['plan_available_time'][l_id] <= 0){
            //     layer.msg('当前任务已经没有剩余排期时间可用!', {icon: 2, time: 1200, anim: 6});
            //     return false;
            // }
            ...
            if(o.hasTime() === l.hasTime()){
                ...
                if( n.allDay && W(i)){
                    ...
                    if((Strack.date_to_unix((s.getEventEnd(n)).format()) / 1000) > Strack.G.calendarTaskRemainderLime['task_remainder_end_time'][l_id]){
                        layer.msg('计划排期不能超过任务截止时间!', {icon: 2, time: 1200, anim: 6});
                        return false;
                    }
                    ...
                    if((Strack.date_to_unix((r.end).format()) / 1000) > Strack.G.calendarTaskRemainderLime['task_remainder_end_time'][l_id]){
                        layer.msg('计划排期不能超过任务截止时间!', {icon: 2, time: 1200, anim: 6});
                        return false;
                    }
                }

                if(jQuery.inArray(r.start.format('YYYY-MM-DD'), Strack.G.calendarLockDateList) >= 0){
                    layer.msg('当前为锁定区域, 禁止编辑!', {icon: 2, time: 1200, anim: 6});
                    return false;
                }

            }else {

                if(jQuery.inArray(l.clone().format('YYYY-MM-DD'), Strack.G.calendarLockDateList) >= 0){
                    layer.msg('当前为锁定区域, 禁止编辑!', {icon: 2, time: 1200, anim: 6});
                    return false;
                }

                if(((Strack.date_to_unix((l.clone()).format()) / 1000) + 600) > Strack.G.calendarTaskRemainderLime['task_remainder_end_time'][l_id]){
                    layer.msg('计划排期不能超过任务截止时间!', {icon: 2, time: 1200, anim: 6});
                    return false;
                }

                Strack.G.calendarTaskRemainderLime['plan_available_time'][l_id] -= 2;
        ...
        computeEventEndResize:function(t, e, n) {
            ...
            var c_time = Strack.current_time();
            if(Strack.G.calendarTaskRemainderLime['task_remainder_end_time'][n.link_id] < c_time){
                layer.msg('当前任务已经超期，请联系负责人!', {icon: 2, time: 1200, anim: 6});
                return false;
            }


            if((Strack.date_to_unix((e.end).format()) / 1000) > Strack.G.calendarTaskRemainderLime['task_remainder_end_time'][n.link_id]){
                layer.msg('计划排期不能超过任务截止时间!', {icon: 2, time: 1200, anim: 6});
                return false;
            }

            var inter_val = (e.end.unix() - e.start.unix()) / 60;

            if(inter_val > 0){
                if(e.end.unix() >= n.end.unix()){

                    // if(Strack.G.calendarTaskRemainderLime['plan_available_time'][n.link_id] < inter_val){
                    //     layer.msg('当前任务已经没有剩余排期时间可用!', {icon: 2, time: 1200, anim: 6});
                    //     return false;
                    // }else {
                    //     // 向下操作
                    //     Strack.G.calendarTaskRemainderLime['plan_available_time'][n.link_id] -= inter_val;
                    // }
        ...
        computeEventResize:function(t, e, n, i) {
            ...
            return r.allDay && W(l) && (r.allDay = !1, o.normalizeEventTimes(r)),
                r[t].add(l), r.start.isBefore(r.end) || (s = this.minResizeDuration || (i.allDay ? o.defaultAllDayEventDuration :o.defaultTimedEventDuration),
                "start" == t ? r.start = r.end.clone().subtract(s) :r.end = r.start.clone().add(s)), r;
        ...
        getEventTimeText:function(t, e, n) {
            ...
            if(null == e){
                e = this.eventTimeFormat;
                if(null == n ){
                    n = this.displayEventEnd;
                    if(this.displayEventTime && t.start.hasTime()){
                        if( n && t.end){
                            return this.view.formatRange(t, e)
                        }else {
                            return t.start.format(e);
                        }
                    }
                }
            }
    ...
    me.mixin({
        ...
        unrenderEvents:function() {
            this.removeSegPopover();
            pe.prototype.unrenderEvents.apply(this, arguments);
        },
        getEventSegs:function() {
        ...
        generateAllDayEventDom: function(data){
            // 渲染
            var dom = '';
            var time_log_dom = '';
            var random_id = Math.floor(Math.random() * 1000000 + 1);
            if(data["is_my_task"]['status'] === "yes"){
                time_log_dom +=  Strack.init_timelog_bnt_dom(data, 'schedule_slider_tg_bnt_'+random_id, 'schedule');
            }else {
                time_log_dom +=  '<a href="javascript:;" class="aign-left fc-h-event-all-day-tg">-</a>';
            }

            // 判断结算按钮显示隐藏
            var af_settlement_bnt = 'display-hide-important',
                jj_settlement_bnt = 'display-hide-important',
                cf_settlement_bnt = 'display-hide-important';
            ...
            var sta_color = data.status_color ? '#'+data.status_color : '#e2e2e2';

            dom += '<div class="fc-day-grid-event fc-h-event fc-h-event-all-day" style="border-left: 2px solid '+data.event_border_color+'">' +
                time_log_dom+
                '<div class="aign-left fc-h-event-all-day-col"><strong>'+StrackLang['Project_Name']+'：</strong>'+data.project_name+'</div>'+
                '<div class="aign-left fc-h-event-all-day-col"><strong>'+StrackLang['Task_Name']+'：</strong>'+data.name+'</div>'+
                '<div class="aign-left fc-h-event-all-day-col" style="text-align: center"><bottun class="ui button basic" style="box-shadow: 0 0 0 1px #'+data.status_color+' inset !important;color: #'+data.status_color+' !important;">'+data.status_name+'</bottun></div>'+
                '<div class="aign-left fc-h-event-all-day-col">' +
                '<div>' +
                '<strong>'+StrackLang['Distributive_Person']+'：</strong>'+data.reviewed_by_user_name+
                '</div>' +
                '<div>' +
                '<strong>'+StrackLang['Executor']+'：</strong>'+data.assignee_user_name+
                '</div>'+
                '</div>'+
                '<div class="aign-left fc-h-event-all-day-col"><strong>'+StrackLang['Cutoff_Time']+'：</strong>'+data.end_time+'</div>'+
                '<div class="aign-left fc-h-event-all-day-col"><strong>'+StrackLang['Remaining_Time']+'：</strong>'+data.remaining_time+'</div>'+
                '<div class="aign-left fc-h-event-all-day-col"><strong>'+StrackLang['Actual_T_Consuming']+'：</strong>'+data.actual_time_consuming+'</div>'+
                '<div class="aign-left fc-h-event-all-day-calculate">' +
                '<div class="item-list-ptn aign-left"><strong>'+StrackLang['Settlement_Time_Consuming']+'：</strong></div>' +
                '<div class="item-list-pt overflow-hide"></div>'+
                Strack.widget_input_dom(data.settlement_time_consuming.config, data.settlement_time_consuming.primary, data.settlement_time_consuming.val, 'schedule', false, data.settlement_time_consuming.param)+
                '</div>'+
                '<div class="aign-left fc-h-event-all-day-bnt">' +
                '<a href="javascript:;" id="'+random_id+'_slider_af_settlement_bnt" class="ui button orange '+af_settlement_bnt+' settlement_bnt_af_'+data['item_id']+'" onclick="Strack.application_for_settlement(this, event)" data-linkid="'+data['item_id']+'" data-moduleid="'+data['module_id']+'" data-projectid="'+data['project_id']+'" data-from="'+random_id+'" data-eq="'+data['assignee_eq_reviewed']+'" data-calculate="'+data['calculate_auto']+'" title="">'+
                Strack.show_application_for_settlement_lang(data.reviewed_by_user_name, data.assignee_user_name) +
                '</a>'+
                '<a href="javascript:;" id="'+random_id+'_slider_jj_settlement_bnt" class="ui button red '+jj_settlement_bnt+' settlement_bnt_jj_'+data['item_id']+'" onclick="Strack.reject_for_settlement(this, event)" data-linkid="'+data['item_id']+'"  data-moduleid="'+data['module_id']+'" data-projectid="'+data['project_id']+'" data-from="'+random_id+'" data-eq="'+data['assignee_eq_reviewed']+'" title="">'+
                StrackLang['Reject_For_Settlement'] +
                '</a>'+
                '<a href="javascript:;" id="'+random_id+'_slider_cf_settlement_bnt" class="ui button green '+cf_settlement_bnt+' settlement_bnt_cf_'+data['item_id']+'" onclick="Strack.confirmation_for_settlement(this, event)" data-linkid="'+data['item_id']+'" data-moduleid="'+data['module_id']+'" data-projectid="'+data['project_id']+'" data-from="'+random_id+'" data-eq="'+data['assignee_eq_reviewed']+'" title="">'+
                StrackLang['Confirmation_For_Settlement'] +
                '</a>'+
                '</div>'+
                '</div>';
        ...
        fgSegHtml:function(t, e) {
            var n,
                ...
                a = !e && t.isEnd && r.isEventResizableFromEnd(s),
            ...
            // if(s.allDay && this.view.name === 'agendaDay'){
            //     // 全天事件独立处理
            //     return this.generateAllDayEventDom(s);
            // }
            ...
            switch (this.view.name) {
                case 'agendaDay':
                    return this.generateAllDayEventDom(s);
                default:
                    if(s.is_repeated_settlement === 'no' && (s.is_all_day === 'no' || jQuery.inArray(this.view.name, ['agendaWeek', 'agendaMonth']) >=0)){
                        t.isStart && (n = this.getEventTimeText(s), n && (c = '<span class="fc-time">' + tt(n) + "</span>"));
                        i = '<span class="fc-title">' + (tt(s.title || "") || "&nbsp;") + "</span>";

                        var item_dom = '<div class="' + u.join(" ") + '"' + (s.url ? ' href="' + tt(s.url) + '"' :"") + (d ? ' style="' + d + '"' :"") + '>' +
                            '<div class="fc-content">' + (this.isRTL ? i + " " + c :c + " " + i) + "</div>" +
                            (l ? '<div class="fc-resizer fc-start-resizer" />' :"") + (a ? '<div class="fc-resizer fc-end-resizer" />' :"") +
                            "</div>";
            ...
            // if(s.is_all_day === 'yes'){
            //     if(jQuery.inArray(this.view.name, ['agendaDay']) >=0){
            //         return this.generateAllDayEventDom(s);
            //     }else {
            //         return '';
            //     }
    ...
    ye.mixin({
        ...
        fgSegHtml:function(t, e) {
            var n, i, r,
                s = this.view,
                o = t.event, l = s.isEventDraggable(o),
            ...
            // s.isMultiDayEvent(o) ? (t.isStart || t.isEnd) && (n = this.getEventTimeText(t), i = this.getEventTimeText(t, "LT"), r = this.getEventTimeText(t, null, !1))
            //     :(n = this.getEventTimeText(o), i = this.getEventTimeText(o, "LT"), r = this.getEventTimeText(o, null, !1));

            var sta_color = o.status_color ? '#'+o.status_color : '#e2e2e2';

            var time_log_dom = '';
            var random_id = Math.floor(Math.random() * 1000000 + 1);
            if(o["is_my_task"]['status'] === "yes"){

                if(jQuery.inArray(parseInt(o.link_id), Strack.G.tempStartTimelogids.ids) >= 0){
                    o.timelog = Strack.G.tempStartTimelogids.status[parseInt(o.link_id)];
                }

                if(o.event_type === 'event'){
                    time_log_dom +=  Strack.init_timelog_bnt_dom({
                        timelog : o.timelog,
                        item_id : o.link_id,
                        module_id : o.base_module_id
                    }, 'schedule_slider_tg_bnt_'+random_id, 'schedule');
                }else {
                    time_log_dom +=  Strack.init_timelog_bnt_dom(o, 'schedule_slider_tg_bnt_'+random_id, 'schedule');
                }
            }else {
                time_log_dom +=  '<a href="javascript:;" class="aign-left fc-h-event-all-day-tg">-</a>';
            }
            ...
            if(s.options.param.rule_update_widget === 'yes' && o.lock_status === 'no') {
                event_copy_bnt += '<a href="javascript:;" class="aign-left fc-h-event-copy" data-id="' + o.id + '"><i class="icon-uniE665 icon-left"></i>复制</a>';
            }

            var del_plan_event_bnt = '';
            if(s.options.param.rule_delete_task_plan === 'yes'  && o.lock_status === 'no') {
                del_plan_event_bnt += '<a href="javascript:;" class="aign-left fc-h-event-del" data-id="'+o.item_id+'"><i class="icon-uniE6DB icon-left"></i></a>';
            }

            d.push('fc-h-event-normal');

            if(o.complete === 'yes'){
                // 增加计划完成状态样式 置灰和文字下划线
                d.push('fc-card-complate');
            }
            ...
            if(o["af_settlement_bnt"] === "yes"){
                // 申请按钮
                af_settlement_bnt = '';
            }

            if(o["jj_settlement_bnt"] === "yes"){
                // 拒绝按钮
                jj_settlement_bnt = '';
            }

            if(o["cf_settlement_bnt"] === "yes"){
                // 确认按钮
                cf_settlement_bnt = '';
            }

            var settlement_dom = '';
            settlement_dom +='<a href="javascript:;" id="'+random_id+'_slider_af_settlement_bnt" class="ui button orange '+af_settlement_bnt+'" onclick="Strack.application_for_settlement(this, event)" data-linkid="'+o['link_id']+'" data-moduleid="'+o['base_module_id']+'" data-projectid="'+o['project_id']+'" data-from="'+random_id+'" data-eq="'+o['assignee_eq_reviewed']+'" data-calculate="'+o['calculate_auto']+'" title="">'+
            StrackLang['Application_For_Settlement'] +
            '</a>'+
            '<a href="javascript:;" id="'+random_id+'_slider_jj_settlement_bnt" class="ui button red '+jj_settlement_bnt+'" onclick="Strack.reject_for_settlement(this, event)" data-linkid="'+o['link_id']+'"  data-moduleid="'+o['base_module_id']+'" data-projectid="'+o['project_id']+'" data-from="'+random_id+'" data-eq="'+o['assignee_eq_reviewed']+'" title="">'+
            StrackLang['Reject_For_Settlement'] +
            '</a>'+
            '<a href="javascript:;" id="'+random_id+'_slider_cf_settlement_bnt" class="ui button green '+cf_settlement_bnt+'" onclick="Strack.confirmation_for_settlement(this, event)" data-linkid="'+o['link_id']+'" data-moduleid="'+o['base_module_id']+'" data-projectid="'+o['project_id']+'" data-from="'+random_id+'" data-eq="'+o['assignee_eq_reviewed']+'" title="">'+
            StrackLang['Confirmation_For_Settlement'] +
            '</a>';

            var e_time = o.event_type === 'event' ? o.base_end_time : o.end_time;

            var item_dom = '<div class="' + d.join(" ") + '"' + (o.url ? ' href="' + tt(o.url) + '"' :"") + ' style="background-color: #FFFFFF;color: #555;border: none;box-shadow: 0 1px 3px 0 #D4D4D5, 0 0 0 1px #D4D4D5;padding: 5px; border-style: solid;border-width: 0 0 0 2px;border-color: '+o.event_border_color+';">' +
                '<div class="fc-content">' +
                '<div class="fc-card-top fc-card-head" style="overflow: hidden;margin-bottom: 10px">' +

                '<div class="fc-card-remaind aign-left" style="background-color: '+o.event_border_color+';">'+o.remaining_time+'</div>' +

                time_log_dom +

                '<div class="fc-card-bnt aign-left">' +
                settlement_dom +
                '</div>'+

                (n ? '<div class="fc-time aign-left" style="margin-left: 10px" data-start="' + tt(r) + '" data-full="' + tt(i) + '"><span>' + tt(n) + "</span></div>" :"") +
                event_copy_bnt +
                del_plan_event_bnt +
                '</div>'+
                '<div class="fc-card-body" style="overflow: hidden;">' +
                (o.project_name ? '<div class="fc-title aign-left" style="margin-right: 10px"><strong>'+StrackLang['Project_Name']+'：</strong>' + tt(o.project_name) + "</div>" :"") +
                (o.title ? '<div class="fc-title aign-left" style="margin-right: 10px"><strong>'+StrackLang['Task_Name']+'：</strong>' + tt(o.title) + "</div>" :"") +
                (o.end_time ? '<div class="fc-title aign-left" style="margin-right: 10px"><strong>'+StrackLang['Cutoff_Time']+'：</strong>' + tt(e_time) + "</div>" :"") +
                (o.reviewed_by_user_name ? '<div class="fc-title aign-left" style="margin-right: 10px"><strong>'+StrackLang['Distributive_Person']+'：</strong>' + tt(o.reviewed_by_user_name) + "</div>" :"") +
                (o.assignee_user_name ? '<div class="fc-title aign-left"><strong>'+StrackLang['Executor']+'：</strong>' + tt(o.assignee_user_name) + "</div>" :"") +
                '</div>'+
                '</div><div class="fc-bg"/>' +
            ...
            //return  '<a class="' + d.join(" ") + '"' + (o.url ? ' href="' + tt(o.url) + '"' :"") + (c ? ' style="' + c + '"' :"") + '><div class="fc-content">' + (n ? '<div class="fc-time" data-start="' + tt(r) + '" data-full="' + tt(i) + '"><span>' + tt(n) + "</span></div>" :"") + (o.title ? '<div class="fc-title">' + tt(o.title) + "</div>" :"") + '</div><div class="fc-bg"/>' + (u ? '<div class="fc-resizer fc-end-resizer" />' :"") + "</a>";
        },
        ...
        generateFgSegHorizontalCss:function(t) {
            ...
            if(this.isRTL){
                e = 1 - s;
                n = r;
                css_r_left = 'marginLeft';
            }else {
                e = r;
                n = 1 - s;
                css_r_left = 'marginRight';
            }
    ...
    Ee.mixin(le), Ee.mixin({
        ...
        buttonText:{
            ...
            prevYear:"prev year",
            nextYear:"next year",
            year:StrackLang["Year"],
            today:StrackLang["Today"],
            month:StrackLang["Month"],
            week:StrackLang["Week"],
            day:StrackLang["Days"]
        },
        buttonIcons:{
    ...
    var Le = jt.AgendaView = Se.extend({
        ...
        renderDates:function() {
            ...
            this.dayGrid && (
                this.dayGrid.setElement(this.el.find(".fc-day-grid")), this.dayGrid.renderDates(),
        ...
        setHeight:function(t, e) {
            ...
            if(!e){
                ...
                if(this.timeGrid.getTotalSlatHeight() < s){
    ...
    return Ut.list = {
        ...
        defaults:{
            buttonText:"list",
            listDayFormat:"LL",
            No_Events_Need_ShowMessage: StrackLang["No_Events_Need_Show"]
        }
    }, Ut.listDay = {
        type:"list",
        duration:{
        ...
        defaults:{
            buttonText: StrackLang["ListWeek"],
            listDayFormat:"dddd",
            listDayAltFormat:"LL"
        }
    }, Ut.listMonth = {
