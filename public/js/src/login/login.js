...
window.onload=function() {
    ...
    Strack_Check.check_browser_version(
        function () {
            ...
            Strack_Check.get_oss_status(function () {
                // 加载第三方服务
                get_third_server();
        ...
        $.ajax({
            ...
            success : function (data) {
                if(data["third_server_status"] === "on"){
                    ...
                    var list_dom = '';
                    var iem_dom = '';
                    data["third_server_list"]["third"].forEach(function (val, index) {
                        // 每5个组成一行
                        count ++;
                        if(count === 5){
                            count = 0;
                            ...
                            iem_dom += server_item_dom(val, 'url');
                        }
                    ...
                    $('.third-party-list .main-list').empty().append(list_dom);

                    var l_count = 0;
                    var l_list_dom = '';
                    var l_iem_dom = '';
                    data["third_server_list"]["ldap"].forEach(function (val, index) {
                        // 每5个组成一行
                        l_count ++;
                        if(l_count === 5){
                            l_count = 0;
                            l_list_dom += server_row_dom(l_iem_dom);
                            l_iem_dom = '';
                        }else {
                            l_iem_dom += server_item_dom(val, 'bnt');
                        }

                        // 结尾
                        if(l_count !== 6 && parseInt(data["third_server_total"]) === index+1){
                            l_list_dom += server_row_dom(l_iem_dom);
                            l_iem_dom = '';
                        }
                    });

                    $('.third-party-list .ldap-list').empty().append(l_list_dom);
                }
            }
        });
    }
     ...
     * @returns {string}
     */
    function server_item_dom(param, type) {
        var dom = '';
        var a_dom = '';

        if(type === 'url'){
            a_dom = '<a href="'+param['url']+'" class="server-core" onclick="obj.select_server(this)" data-type="'+param["type"]+'" data-id="'+param["id"]+'" data-name="'+param["name"]+'">';
        }else {
            a_dom = '<a href="javascript:;" class="server-core" onclick="obj.select_server(this)" data-type="'+param["type"]+'" data-id="'+param["id"]+'" data-name="'+param["name"]+'">';
        }
