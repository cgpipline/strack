$(function () {
    ...
    $('#ldap_server_list').datagrid({
        ...
        onLoadSuccess: function (data) {
            ...
            var open_ldap_val = 0,
                open_qq_login_val = 0,
                open_strack_union_val = 0;

            if(data["ldap_open"]){
                open_ldap_val = 1;
            }

            if(data["qq_login_open"]){
                open_qq_login_val = 1;
            }

            if(data["strack_union_open"]){
                open_strack_union_val = 1;
            }
            ...
            Strack.init_open_switch({
                dom: '#open_strack_union_login',
                value: open_strack_union_val,
                onText: StrackLang['Switch_ON'],
                offText: StrackLang['Switch_OFF'],
                width: 100,
            }, function (status) {
                save_login_setting({
                    type: 'strack_union_open',
                    status: status
                });
            });
     ...
     * @param data
     */
    function save_login_setting(data) {
        $.ajax({
            type : 'POST',
            url : SignMethodPHP['saveLoginMethodSetting'],
            data : JSON.stringify(data),
            dataType : 'json',
            contentType: "application/json",
            beforeSend : function () {
