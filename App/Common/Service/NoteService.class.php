...
class NoteService
{
    ...
    public function deleteNote($param, $mode = "details")
    {
        ...
        if (!$resData) {
            ...
            TPRequest::$serviceOperationResData = $param;
    ...
    public function addNote($param, $moduleId)
    {
        ...
        try {
            ...
            TPRequest::$serviceOperationResData = $this->getFormatNoteData($noteData, $moduleId);
     ...
     * @return array
     */
    public function addTaskRejectNote($param)
    {
        $baseModel = new BaseModel();
        $taskData = $baseModel->field('id,project_id')->where(['id' => $param['link_id']])->find();
        $noteModuleId = C("MODULE_ID")["note"];

        $images = [];
        $mediaServer = [];
        if(!empty($param['media_data'])){
            $images = $param['media_data']['images'];
            $mediaServer = $param['media_data']['media_server'];
        }
        ...
        $resData = $this->addNote($addParam, $noteModuleId);
    ...
    public function modifyNote($param, $moduleId)
    {
        ...
        try {
            ...
            TPRequest::$serviceOperationResData = $noteData = $this->getFormatNoteData($noteData, $moduleId);
            return success_response($noteModel->getSuccessMassege(), $noteData);
        } catch (\Exception $e) {
            $noteModel->rollback(); // 事物回滚
            // 修改数据失败错误码 002
