...
class LoginController extends VerifyController
{
    ...
    public function index()
    {
        ...
        // 判断是否启用OSS单点登录
        $this->assign('oss_open', C('STRACK_UNION_CONFIG')['oss_open']);
     ...
     * @return mixed
     */
    public function getOssStatus()
    {

        $authHttpCode = check_http_code(C("STRACK_UNION_CONFIG")["base_url"]);
        $status = 404;
        $userOssStatus = [];
        ...
        if ($authHttpCode === 200) {
            ...
            $oauthObject = $loginService->initOauthObject('strack');
            $param = $this->request->param();

            // 存储设备唯一码
            session('Device_Unique_Code', $param['device_unique_code']);

            try {
                $status = 200;
                $state = md5(create_uuid());
                session($oauthObject['stateName'], $state);
                $userOssStatus = $oauthObject['oAuth']->getUserOssStatus($param['device_unique_code'], $state);
            } catch (\Exception $e) {
    ...
    {
        if (C('STRACK_UNION_CONFIG')['oss_open'] === 'yes' && session('?Device_Unique_Code')) {
            // 同步注册OSS统一登陆状态
            $loginService = new LoginService();
            $oauthObject = $loginService->initOauthObject('strack');
            $deviceUniqueCode = session('Device_Unique_Code');

            try {
                $oauthObject['oAuth']->cancelUserOssStatus($deviceUniqueCode);
            } catch (\Exception $e) {
            }
        }
