...
class ViewController extends VerifyController
{
     ...
     * @return array
     */
    protected function checkUserMenuAuth($menuList)
    {
        // 菜单code映射，菜单权限code
        $menuMap = [
            ...
            'admin' => 'admin_manage',
            'my_account' => 'personal_account',
            'schedule' => 'my_schedule_index',
            'report' => 'report_index'
        ];

        $topMenuViewAuth = $this->authService->getMenuPermission("top_menu");
        $topMainMenuViewAuth = $this->authService->getMenuPermission("top_main_menu");
        $adminMenuViewAuth = $this->authService->getMenuPermission("admin_menu");
        $accountTopMenuViewAuth = $this->authService->getMenuPermission("account_top_menu");
        $topMenuAuth = array_merge($topMenuViewAuth, $topMainMenuViewAuth, $adminMenuViewAuth, $accountTopMenuViewAuth);

        $newMenuList = [];
        foreach ($menuList as $menuItem) {
            if ($topMenuAuth[$menuMap[$menuItem]] === 'yes') {
    ...
    {
        $param = $this->request->param();
        if (in_array($param["tab_param"]["type"], ["horizontal_relationship"])) {
     ...
     * 3、把数据返回给后端
     */
    public function frontendCheckCouldDiskServer()
    {
        $param = $this->request->param();
        if (array_key_exists('type', $param)) {
            ...
            if ($param['type'] === 'check') {
                // 获取check
                return $diskService->getCloudDiskEndpoints();
            } else {
                return $diskService->getCloudDiskUrlByFrontend($param['data'], $param['fast_endpoints'], $param['param']);
            }
        } else {
            // 请求类型
            return success_response('Request type parameter does not exist.');
        }
    }
}
