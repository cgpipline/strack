...
class VerifyController extends Controller
{
    ...
    // 当前源路由控制器
    protected $currentSourceController;
     ...
     * @param bool $isAssign
     */
    protected function getPageAuthRules($isAssign = true)
    {
     ...
     * @param $token
     */
    protected function verifyApiPermission($token)
    {
        // 实例化权限服务对象
    ...
    /**
     * 发送belong_system名称到前端
     */
    protected function assignBelongSystemName()
    {
        $this->assign([
    ...
    protected function verifyApiToken()
    {
        if ($this->currentController !== "login") {
            ...
            if (array_key_exists("token", $this->header) && isset($this->header["token"])) {
                ...
                // 验证页面权限
                $this->verifyApiPermission($this->header["token"]);
            } else {
                // token不存在
    ...
    protected function _initialize()
    {
        ...
        $this->currentSourceController = CONTROLLER_NAME;
