 ...
 * @throws Exception
 */
function auto_fill_code($val = '')
{
    if (!empty($val)) {
        return $val;
    }
    return string_random(8) . '_' . time();
}
 ...
 * @return int|string
 */
function duration_format_show($val)
{
    if (!empty($val)) {
        if (strpos($val, ':') !== false) {
            list($hour, $minute) = explode(':', $val);
            $hourFormat = intval($hour);
            $minuteFormat = intval($minute);
            return "{$hourFormat}小时{$minuteFormat}分钟";
        } else {
            $hour = intval(floatval($val) / 60);
            $minute = round(floatval($val) % 60);
 ...
 * @return int|string
 */
function convert_time_to_minute($val)
{
    if (!empty($val)) {
        if (strpos($val, ':') !== false) {
            list($hour, $minute) = explode(':', $val);
            $hourFormat = intval($hour);
            $minuteFormat = intval($minute);
            return ($hourFormat * 60) + $minuteFormat;
        } else {
            $hour = intval(floatval($val) / 60);
            $minute = round(floatval($val) % 60);
            return ($hour * 60) + $minute;
 ...
 * 计算计划可用时间(分钟)
 * @param $endTime
 * @param $estimateAvailableTime
 * @return float|int
 */
function generate_available_plan_time($endTime, $estimateAvailableTime)
{
    $currentTime = time();
    ...
    if ($interval > 0) {
        ...
        if ($intervalMin > $estimateAvailableTime) {
            return $estimateAvailableTime;
        }
    ...
    switch ($page) {
        ...
        case "home_project_page":
            ...
            if (is_array($getUrlParam)) {
                ...
                $urlParam["project_id"] = $getUrlParam;
            }
            break;
 ...
 */
function remove_url_agreement($url)
{
...
function get_ajax_url_referer($referer)
{
    ...
    // 去掉请求头
    $baseUrlOutAgree = remove_url_agreement($baseUrl);
    $refererOutAgree = remove_url_agreement($referer);
    $refererParam = str_replace($baseUrlOutAgree, '', $refererOutAgree);
 ...
 * @return false|string
 */
function fill_default_time($value = '')
{
    if (!empty($value)) {
        return $value;
    }
...
{
    return [
        ['id' => 'un_evaluated', 'name' => L('Un_Evaluated')],
 ...
 * @return bool
 */
function check_time_in_range($startTime, $endTime, $range = [])
{
    if (!empty($range)) {
        ...
        if ($endTime >= $range[0] && $endTime <= $range[1]) {
            // 结束时间在范围内
            return true;
        }
 ...
 * @return string
 */
function generate_cloud_disk_url($url, $param)
{
    return $url . $param['type'] . '/' . $param['link_uuid'] . '/' . $param['user_id'];
}
 ...
 * @return mixed|string
 */
function get_time_week($time, $inner = 0, $type = 'en')
{
    $weekArrayEN = ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'];
    $weekArrayNumber = [1, 2, 3, 4, 5, 6, 7];

    switch ($type) {
        case 'number':
            $week = $weekArrayNumber[date("w", $time + $inner * 86400)];
            break;
        case 'en':
        default:
            $week = $weekArrayEN[date("w", $time + $inner * 86400)];
            break;
    }
    return $week;
}
 ...
 * @return int
 */
function check_param_int_empty($param, $field)
{
    return !empty($param[$field]) ? $param[$field] : 0;
}
