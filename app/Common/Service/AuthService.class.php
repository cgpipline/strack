...
class AuthService
{
     ...
     * @return string
     */
    protected function getPageAuthPermission($pageAuthData, $onlyJudgeRole = false, $returnString = false, $moduleName = '', $from = '')
    {
    ...
    protected function checkWhetherTopToolPanel($param)
    {
        ...
        // 查找他们的儿子
        $authParentIds = array_column($topPanelData, 'id');
        $childData = $pageAuthModel->where(["parent_id" => ['IN', join(',', $authParentIds)]])->select();
                ...
                foreach ($this->allPageLinkAuthDataDict[$value["id"]] as $groupId) {
                    foreach ($this->allAuthGroupDataDict[$groupId]["rules"] as $rules) {
                        if (md5(strtolower($rules["rules"])) === $param["source_node"]) {
                            ...
                            $topPanelPermission["permission"] = $this->getPageAuthPermission($value);
                            break;
                        }
                    }
                }
    ...
    public function getUserRoleData($userId, $projectId = 0)
    {
        ...
        if ($projectId > 0) {
            ...
            if ($roleId > 0) {
                ...
                $projectData = $projectModel->field('public,created_by')->where(["id" => $projectId])->find();
                if ($projectData['public'] === "no" && $projectData['created_by'] != $userId) {
                    // 为私有项目，而且也不是创建者
                    $notInProjectMember = false;
                }
            }
     ...
     * @return array
     */
    public function getRoleDataRangeAuthUserIds($userId, $page, $projectId = 0)
    {
        if (in_array((int)$userId, [1, 2])) {
            // 超级管理员和strack账号不需要判断权限
            return [];
        }
        $roleData = $this->getUserRoleData($userId, $projectId);

        // 获取当前页面配置的数据权限，没有记录则只能看自己范围数据
        $authAccessModel = new AuthAccessModel();
        $authAccessData = $authAccessModel->where([
            "role_id" => $roleData["id"],
            "page" => $page,
            "type" => "data_range"
        ])->find();
        ...
        $userIds = [];
        if (!empty($authAccessData)) {
            switch ($authAccessData['permission']) {
                case 'my_charge_team':
                    // 我负责的团队
                    $userService = new UserService();
                    $userIds = $userService->getMyChargeTeamUserIds($userId);
                    break;
                case 'all_data':
                    // 所有数据
                    $userIds = [];
    ...
    public function verifyPagePermission($param)
    {
        ...
        switch ($param["type"]) {
            ...
            case "api": //验证api路由权限
                ...
                if (array_key_exists($pageAuthData["id"], $this->allPageLinkAuthDataDict)) {
                    return $this->getPageAuthPermission($pageAuthData, false, false, $param["controller"], 'api');
                }
     ...
     * @return array
     */
    protected function getRelatedModuleList()
    {
        $moduleList = [];
        ...
        foreach ($variableData['rows'] as $variableItem) {
            if ($variableItem['config']['relation_module_id'] === C('MODULE_ID')['user']) {
                $moduleList[] = [
                    'id' => $variableItem['config']['code'],
                    'lang' => $variableItem['config']['name']
                ];
            }
        }
     ...
     * @return array
     */
    public function getAuthPageDataRangeModuleData()
    {
        $moduleList = [
            'fixed_module' => [
                'lang' => L("Fixed_Module"),
...
//                    ]
            ...
            'task_related_user' => [
                'lang' => L("Task_Related_User_Module"),
                'list' => $this->getRelatedModuleList()
            ],
        ];
     ...
     * @return mixed
     */
    public function getAuthDataRangeRules($param)
    {
        switch ($param["class_type"]) {
            case "task_related_user":
                $roleAuthCode = 'view_related_to_me';
                $pageAuthFilter = [
                    'category' => 'data_range',
        ...
        // 获取当前角色、页面设置的数据权限
        $authAccessModel = new AuthAccessModel();
        $roleAuthId = $authAccessModel->where($authAccessFilter)->getField('auth_id');
        ...
        // 获取权限列表
        $pageAuthModel = new PageAuthModel();
        $authRules = $pageAuthModel->where($pageAuthFilter)->select();

        foreach ($authRules as &$authRule) {
            if (($roleAuthExit && $authRule['id'] == $roleAuthId) || (!$roleAuthExit && $authRule['code'] == $roleAuthCode)) {
                $authRule['checked'] = 'yes';
            } else {
                $authRule['checked'] = 'no';
            }
        }
    ...
    public function getUserAllFieldPermission($userId)
    {
        ...
        foreach ($allAuthFieldDataDict as $moduleCode => $fields) {
     ...
     * @return bool
     */
    public function checkBaseRelatedUserPermission($field, $mode, $baseHorizontalUserData = [])
    {
        if (!empty($baseHorizontalUserData)) {
            if (in_array($mode, $baseHorizontalUserData)) {
                return true;
            }
        }
        ...
        if (array_key_exists($param["param"]["role_rule_tab"], $param["config"])) {
            ...
            try {
                foreach ($param["config"][$param["param"]["role_rule_tab"]] as $authItem) {
                    ...
                    switch ($param["param"]["role_rule_tab"]) {
                        case 'view':
                            $filter = [
