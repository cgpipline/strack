...
class TemplateService
{
        ...
        foreach ($variableData as $item) {
            if ($isBeRelated) {
                $tagDataVariable = [
                    ...
                    'tab_id' => 'be_' . $item["code"] . '_' . $item["code"],
                    'module_code' => $item["code"],
                    'dst_module_code' => $moduleIdMapData[$item["dst_module_id"]]["code"],
                    'module_type' => $item["type"],
                    'table' => get_table_name($item["code"]),
                    'name' => $item["name"],
                    'horizontal_type' => $item["type"]
                ];
            } else {
                $tagDataVariable = [
    ...
    public function getProjectNavModuleList($templateId)
    {
        ...
        // 如果temple_id > 0则判断当前模块是否被启用
        if ($templateId > 0) {
            $optionsService = new OptionsService();
            $projectModuleSettings = $optionsService->getOptionsData("project_module_settings");

            if(!empty($projectModuleSettings)){
                $newProjectNavModuleList = [];
                foreach ($projectNavModuleList as $item) {
                    if (!empty($projectModuleSettings[$item['code']]) && $projectModuleSettings[$item['code']]['selected'] === 'yes') {
                        $newProjectNavModuleList[] = $item;
                    }
                }

                return $newProjectNavModuleList;
            }
        }
