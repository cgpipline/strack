...
class HorizontalService
{
    ...
    public function modifyHRelationDestData($param)
    {
        ...
        if (!empty($param["up_data"]["delete"])) {
            ...
            if ($param['param']['horizontal_type'] === 'entity_child') {
                $entityModel = new EntityModel();

                $resData = $entityModel->updateWidget([
                    "id" => ["IN", join(",", $param["up_data"]["delete"])],
                    'parent_id' => 0,
                    'parent_module_id' => 0
                ]);
                ...
                if (!$resData) {
                    // 删除水平关联失败错误码 003
                    throw_strack_exception($entityModel->getError(), 224002);
                }
            } else {
        ...
        if (!empty($param["up_data"]["add"])) {
            ...
            if ($param['param']['horizontal_type'] === 'entity_child') {
                ...
                $resData = $entityModel->updateWidget([
                    "id" => ["IN", join(",", $param["up_data"]["add"])],
                    'parent_id' => $param["param"]["src_link_id"],
                    'parent_module_id' => $param["param"]["src_module_id"]
                ]);
     ...
     * @return mixed
     */
    public function getHorizontalServiceData($srcModuleId, $dstModuleId, $variableId, $srcLinkId)
    {
        $horizontalModel = new HorizontalModel();
        $dstLinkId = $horizontalModel
            ->where([
                'src_module_id' => $srcModuleId,
