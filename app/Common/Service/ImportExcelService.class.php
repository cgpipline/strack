...
//        $modelClass = new Model();
        ...
        try {
            ...
            foreach ($columnFieldData as $columnItem) {
                // 保存主表
                if(!empty($columnItem["master_data"]['priority'])){
                    $columnItem["master_data"]['priority'] = get_priority_en($columnItem["master_data"]['priority']);
                }
                            ...
                            if ($relationKey == $moduleData["code"]) { // 保存自定义字段
                                ...
                                foreach ($relationItem as &$variableItem) {
                                    ...
                                    if ($relationModel->where($customFilter)->count() > 0) {
                                        //获取自定义数据
                                        $valueList = $variable->getWidgetData($variableItem["variable_id"]);
                                        if(!empty($valueList) && !empty($variableItem["value"])){
                                            $value = '';
                                            foreach ($valueList as $v){
                                                if($v['name'] == $variableItem["value"]){
                                                    $value = $v['id'];
                                                }
                                            }
                                        ...
                                        //获取自定义数据
                                        $value_list = $variable->getWidgetData($variableItem["variable_id"]);
                                        if(!empty($value_list) && !empty($variableItem["value"])){
                                            $value = '';
                                    ...
                                    //判断字段是否为水平关联（horizontal_relationship）
                                    if(!empty($variableItem['variable_id']) && !empty($variableItem['value'])){
                                        $var_info = $variableItem;
                                        $var_info['src_link_id'] = $masterData['id'];
                                        $relat = $variable->getVariableInfo($var_info,$param["param"]['project_id']);
                                        if($relat['code'] === 1){
                                            throw new \Exception($relat['message']);
                                        }
                                    }
                                }
                            } else {
