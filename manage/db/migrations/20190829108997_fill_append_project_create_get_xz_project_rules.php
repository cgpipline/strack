...
class FillAppendProjectCreateGetXzProjectRules extends AbstractMigration
{
    /**
     * Change Method.
     *
    ...
    public function up()
    {
        $authNode = [
            'name' => '获取协作1.0关联项目列表',
            'code' => 'get_xz_project_combobox',
            'lang' => 'Get_Xz_Project_Combobox',
            'type' => 'route',
            'module' => 'page',
            'project_id' => 0,
            'module_id' => 0,
            'rules' => 'Home/Widget/getXzProjectCombobox',
            'uuid' => Webpatser\Uuid\Uuid::generate()->string
        ];

        $this->table('strack_auth_node')->insert($authNode)->save();

        $authGroupData = $this->fetchRow('SELECT id FROM strack_auth_group WHERE code="project_create" and type="url"');
        $authGroupNode = [
