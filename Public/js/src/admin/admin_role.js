$(function () {
    obj = {
        ...
        select_module: function (i) {
            ...
            var id = $(i).data("id");
            var class_type = $(i).data("classtype");
                ...
                for (var i in data) {
                    ...
                    if(data[i]["list"]){
                        data[i]["list"].forEach(function (val) {
                            mlist += module_list_dom(val, tab_name, data[i], 'field', i);
                        });
                    }
                }
     ...
     * @param tab_name
     */
    function get_auth_data_range_module(tab_name) {
        $.ajax({
            ...
            dataType: 'json',
            beforeSend: function () {
                $('#data_range_module_list').append(Strack.loading_dom('white', '', 'data_range'));
            },
            success: function (data) {
                var mlist = '';
                for (var i in data) {
                    ...
                    mlist += '<li class="module-items-title">' + data[i]["lang"] + '</li>';
                    data[i]["list"].forEach(function (val) {
                        mlist += module_list_dom(val, "data_range", data[i], 'data_range', i);
                    });
                }
                $("#data_range_module_list").empty().append(mlist);
                $('#st-load_data_range').remove();
            }
        });
    }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i in data) {
                    mlist += '<li class="module-items-title">' + data[i]["lang"] + '</li>';
                    data[i]["list"].forEach(function (val) {
                        mlist += module_list_dom(val, "admin", data[i], 'page', i);
                    });
                }
     ...
     * @returns {string}
     */
    function module_list_dom(data, tab, param, type, class_type) {
        var list = '', icon_dom = '';
        ...
        list += '<a href="javascript:;" class="nation-items" onclick="obj.select_module(this)" data-id="' + data["id"] + '" data-type="' + type + '" data-tab="'+tab+'" data-classtype="'+class_type+'">' +
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                switch (tab){
                    ...
                    case "data_range":
                        // 数据范围
                        role_rule_tab.push("data_range");
                        $('#data_range_rules_list')
                            .empty()
                            .append(
                                generate_data_range_dom(param['id'], data)
                            );
                        break;
                }
     ...
     * @returns {string}
     */
    function generate_data_range_dom(tab, data) {
        var dom = '';
        data.forEach(function (val) {
            dom += '<div class="rr-data-r-item">';
            if(val["checked"] === 'yes'){
                dom += '<input type="radio" name="'+tab+'" checked="checked" data-page="'+tab+'" data-authpage="'+val["page"]+'" data-authid ="'+val["id"]+'" data-authcode ="'+val["code"]+'">';
            }else {
                dom += '<input type="radio" name="'+tab+'"  data-page="'+tab+'" data-authpage="'+val["page"]+'" data-authid ="'+val["id"]+'" data-authcode ="'+val["code"]+'">';
            }
            dom += StrackLang[val["lang"]];
            dom +='</div>';
        });
    ...
    function save_module_auth(role_param) {
        ...
        // 获取数据范围权限数据
        if($.inArray("data_range", role_rule_tab) >= 0){
            var data_range_config = [];
            $('#data_range_rules_list input').each(function () {
                if ($(this).prop("checked")) {
                    var r_auth_id = $(this).data('authid'),
                        r_auth_code = $(this).data('authcode'),
                        page = $(this).data('page'),
                        auth_page = $(this).data('authpage');
                    ...
                    switch (auth_page) {
                        case 'task_related_user':
                            r_param['page'] = auth_page;
                            r_param['param'] = page;
                            role_param['auth_param'] = page;
                            break;
                        default:
                            r_param['page'] = page;
                            role_param['auth_param'] = '';
                            break;
                    }

                    data_range_config.push(r_param);
                }
            });

            rule_config["data_range"] = data_range_config;
        }
    ...
    function switch_auth_tab(tab_name, role_id) {
        ...
        var $data_range_panel = $("#data_range_panel");
        ...
        switch (tab_name) {
            ...
            case "data_range":
                // 数据范围权限
                $data_range_panel.addClass("active");
                get_auth_data_range_module(tab_name);
                break;
        }
    }
});
