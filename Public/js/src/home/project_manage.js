$(function () {
            ...
            if(options !== "all"){
                switch (from){
                    ...
                    case "project_time":
                        ...
                        break;
                    case "project_belong":
                        Gfilter["project_belong"] = options;
                        // 把项目状态缓存
                        Strack.save_local_storage('project_belong', options);
                        break;
                }
            }else {
                Gfilter["project_status"] = [];
                Gfilter["project_time"] = "";
                Gfilter["project_belong"] = "";
                Strack.save_local_storage('project_belong', '');
            }
    ...
    var project_belong_cache =  Strack.read_local_storage('project_belong');

    // 默认我的项目
    var project_belong_val = project_belong_cache === null ? 'my' : project_belong_cache;

    var Gfilter = {
        "project_status" : [],
        "project_time" : "",
        "project_belong" : project_belong_val,
        "name" : ""
    };

    if(project_belong_val === 'my'){
        Strack.toggle_item_icon($("#project_filter_my")[0], 'project_belong');
    }else {
        Strack.toggle_item_icon($("#project_filter_all")[0], 'project_belong');
    }
    ...
    function project_card_dom(data) {
        ...
        // 默认跳转到任务页面
        var url = Strack.remove_html_ext(ProjectPHP["project_base"])+ '/' + data["id"] + '.html';
